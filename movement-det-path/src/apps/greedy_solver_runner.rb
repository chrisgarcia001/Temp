...
def print_usage
  puts 'Usage: > ruby greedy_solver_runner.rb <params file> ' + "\n"
end
...
Dir::foreach(input_dir) do |f|
  if f =~ /.json$/
    ...
    sol = greedy_solve(problem)
    sol[:problem_name] = f.split('')[0, f.split('').length - 5].join('')
    ofile = "greedy-" + f.split('')[0, f.split('').length - 5].join('') + ".json"
    puts "Done! \nWriting File: " + ofile + ', Elapsed Time: ' + sol[:elapsed_time].to_s
    write(JSON.generate(sol), File.join(output_dir, ofile))
    solutions << sol
  end
end
hashlist_to_csv solutions, File.join(output_dir, 'summary-greedy.csv'), [:problem_name, :objective_func, :elapsed_time]
