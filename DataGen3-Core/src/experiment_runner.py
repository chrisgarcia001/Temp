...
class ProblemExecutor:
	...
	# For a full problem name, return its prefix/type.	
	def prob_prefix(self, full_prob_name):
		return full_prob_name.split('_')[0]
	...
	def solve_instance(self, problem_id):
		...
		return {self.prob_name('', 'init', self.problem_set_name, problem_id):init_sol_data,
				...
				self.prob_name('', 'cross', self.problem_set_name, problem_id):cross_sol_data}
	
	# Build and write a summary CSV file for the result set.
	def write_results_summary(self, results):
		sorted_keys = sorted(results.keys())
		base_text = ['Problem ID, Objective, Elapsed Time']
		cross_text = ['Problem ID, Quality, Stability']
		print('Generating results summary files...')
		for key in sorted_keys:
			if self.prob_prefix(key) == 'cross':
				cross_text.append(','.join([key, results[key]['quality'], results[key]['stability']]))
			else:
				base_text.append(','.join([key, results[key]['objective'], results[key]['time']]))
		write_file("\n".join(base_text), self.solution_path + '/base_' + self.problem_set_name + '_summary.csv')
		write_file("\n".join(cross_text), self.solution_path + '/cross_' + self.problem_set_name + '_summary.csv')
		print('Done!')
